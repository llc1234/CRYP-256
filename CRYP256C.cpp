...
public:
    ...
    void start(int argc, char* argv[]) {
        if (argc != 3) {
            std::cerr << "Usage: sudo ./CRYP256C <filename/.txt/.png/.jpg> <key/sha640>" << std::endl;
            std::cerr << "Example: sudo ./CRYP256C cat.png 3f307c4a23b754bc8e6f874a4b49303a10" << std::endl;
            std::exit(1);
        }

        filename = argv[1];
        key_generator(hash128_string(argv[2]));
        en_of_de();
    }
};

int main(int argc, char* argv[]) {
    CRYP256C().start(argc, argv);
    return 0;
}
